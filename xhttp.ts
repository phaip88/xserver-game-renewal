...
const UUID: string = Deno.env.get("UUID") || "b59eb4b5-4ea0-44f3-8ea9-fd7f07fff28f";
const SUB_PATH: string = Deno.env.get("SUB_PATH") || "zmm";  // Get subscription path
const XPATH: string = Deno.env.get("XPATH") || "xhttp";      // Node path
const DOMAIN: string = Deno.env.get("DOMAIN") || "lucky9.deno.dev";         // The domain name assigned by /deno is required, without the https://prefix, for example: xxxx.deno.dev
...
function parse_uuid(uuid: string): Uint8Array {
  ...
  for (let index = 0; index < 16; index++) {
    r[index] = parseInt(uuid.substring(index * 2, index * 2 + 2), 16);
  }
  return r;
}
...
async function parse_header(
  ...
  try {
    ...
    throw new Error(`read vless header error: ${error.message}`);
  } finally {
    reader.releaseLock();
  }
}
...
class Session {
  ...
  _startDownstreamResponse(): void {
    ...
    try {
      ...
      this.remote!.readable.pipeTo(this.currentStreamRes.writable).catch((_err: unknown) => {
        // Connection closed
      });
    } catch (_err) {
      this.cleanup();
...
// ISP detection - use environment variable or default
const ISP = Deno.env.get("ISP") || "Deno-Deploy";

// IP detection - use DOMAIN directly to avoid startup delays
const IP = DOMAIN;
